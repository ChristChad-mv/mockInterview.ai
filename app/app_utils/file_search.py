...
def upload_resume_to_store(user_id: str, file_path: str, original_filename: str):
    """
    Uploads a resume file to the global store with user_id metadata.
    """
    client = get_gemini_client()
    store = ensure_global_store_exists(client)
    
    logger.info(f"Uploading {original_filename} (User: {user_id}) to global store {store.name}")
    
    operation = client.file_search_stores.upload_to_file_search_store(
        file=file_path,
        file_search_store_name=store.name,
        config={
            'display_name': f"CV_{user_id[:8]}_{original_filename}",
            'custom_metadata': [
